...
new:AddLabel("完全修复")
new:AddLabel("修复时间12月20号")
new:AddLabel("7月份修复源 -- MR-S")
...
local ZZ = Tabs.Main:AddRightGroupbox('切换服务器')
...
local KillerSurvival = Tabs.zdx:AddRightGroupbox('传送修机')
...
local ZZ = Tabs.mhg:AddRightGroupbox('菜鸟皮肤')
...
ZZ:AddButton({
    ...
    HoverText = "清除菜鸟风格的皮肤效果，恢复默认外观"
})


local ZZ = Tabs.mhg:AddLeftGroupbox('访客1337皮肤')
...
ZZ:AddButton({
    ...
    HoverText = "清除所有皮肤效果，恢复默认外观"
})



local ZZ = Tabs.mhg:AddLeftGroupbox('谢德皮肤')
...
local function dressCharacter()
    ...
    isDressed = true
    ZZ:UpdateButton("应用皮肤", {Text = "清除皮肤"})
end
...
local function cleanupDress()
    ...
    isDressed = false
    ZZ:UpdateButton("谢德皮肤", {Text = "应用皮肤"})
end

ZZ:AddButton("应用皮肤", {
    Text = isDressed and "清除皮肤" or "应用皮肤",
    Func = function()
        if isDressed then
            cleanupDress()
        else
...
local ZZ = Tabs.mhg:AddRightGroupbox('黑手党皮肤')
...
ThemeManager:SetFolder("NOL7月源")
SaveManager:SetFolder("NOL7月源/配置")
SaveManager:SetSubFolder("配置") -- if the game has multiple places inside of it (for example: DOORS)
