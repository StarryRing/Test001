...
-- 获取指定年份的节日日期
local function getFestivalDates(year)
    -- 春节（农历正月初一，这里使用近似计算）
    -- 2025年春节是1月29日，2026年春节是2月17日
    local springFestivalDate
    if year == 2025 then
        springFestivalDate = os.time({year = 2025, month = 1, day = 29, hour = 0, min = 0, sec = 0})
    else
        springFestivalDate = os.time({year = 2026, month = 2, day = 17, hour = 0, min = 0, sec = 0})
    end
    
    -- 除夕（春节前一天）
    local newYearsEveDate = springFestivalDate - 86400
    
    -- 元宵节（农历正月十五，春节后15天）
    local lanternFestivalDate = springFestivalDate + (86400 * 15)
    
    -- 元旦（1月1日）
    local newYearDate = os.time({year = year, month = 1, day = 1, hour = 0, min = 0, sec = 0})
    
    -- 下一个元旦（如果当前已过，则显示下一年的）
    local nextNewYearDate
    local currentTime = os.time()
    if currentTime >= newYearDate then
        nextNewYearDate = os.time({year = year + 1, month = 1, day = 1, hour = 0, min = 0, sec = 0})
    else
        nextNewYearDate = newYearDate
    end
    
    return {
        springFestival = springFestivalDate,
        newYearsEve = newYearsEveDate,
        lanternFestival = lanternFestivalDate,
        newYear = nextNewYearDate
    }
end
...
-- 检查是否是同一年份的节日
local function getNextFestivalTime(targetDate)
    local currentTime = os.time()
    local targetTime = targetDate
    
    -- 如果节日已经过去，找下一年的
    if currentTime >= targetTime then
        -- 如果是春节相关节日，加一年（春节每年日期不同）
        return nil  -- 需要特殊处理
    end
...
-- 创建倒计时标签
local timeLabels = {
    currentTime = infoSection:Label("当前时间: 加载中..."),
    springFestival = infoSection:Label("春节倒计时: 加载中..."),
    newYear = infoSection:Label("元旦倒计时: 加载中..."),
    newYearsEve = infoSection:Label("除夕倒计时: 加载中..."),
    lanternFestival = infoSection:Label("元宵节倒计时: 加载中...")
}
...
local function updateTimeDisplay()
    ...
    -- 获取节日日期
    local festivals2025 = getFestivalDates(2025)
    local festivals2026 = getFestivalDates(2026)
    
    -- 计算各节日倒计时
    local function calculateCountdown(targetDate, festivalName)
        local diff = targetDate - currentTime
        if diff > 0 then
            return formatTimeDifference(diff)
        else
            -- 检查2026年的节日
            local nextYearDate
            if festivalName == "springFestival" then
                nextYearDate = festivals2026.springFestival
            elseif festivalName == "newYearsEve" then
                nextYearDate = festivals2026.newYearsEve
            elseif festivalName == "lanternFestival" then
                nextYearDate = festivals2026.lanternFestival
            elseif festivalName == "newYear" then
                nextYearDate = os.time({year = now.year + 1, month = 1, day = 1, hour = 0, min = 0, sec = 0})
            end
            
            if nextYearDate then
                diff = nextYearDate - currentTime
                if diff > 0 then
                    return formatTimeDifference(diff)
                else
                    return "已过"
    ...
    return {
        currentTime = "当前时间: " .. timeStr,
        springFestival = "春节倒计时: " .. calculateCountdown(festivals2025.springFestival, "springFestival"),
        newYear = "元旦倒计时: " .. calculateCountdown(festivals2025.newYear, "newYear"),
        newYearsEve = "除夕倒计时: " .. calculateCountdown(festivals2025.newYearsEve, "newYearsEve"),
        lanternFestival = "元宵节倒计时: " .. calculateCountdown(festivals2025.lanternFestival, "lanternFestival")
    }
end
...
task.spawn(function()
    while task.wait(1) do
        ...
        -- 更新所有标签文本
        if timeLabels.currentTime then
            timeLabels.currentTime.Text = times.currentTime
        end
        if timeLabels.springFestival then
            timeLabels.springFestival.Text = times.springFestival
        end
        if timeLabels.newYear then
            timeLabels.newYear.Text = times.newYear
        end
        if timeLabels.newYearsEve then
            timeLabels.newYearsEve.Text = times.newYearsEve
        end
        if timeLabels.lanternFestival then
            timeLabels.lanternFestival.Text = times.lanternFestival
        end
    end
end)
